...
void get_backup_filename(char *backup_filename, char *original_filename, char *dst_file_path); //backup file name
void backup_file(char *file_path, char *backup_dir_path, char *backup_dir); //file backup
void backup_directory(char *dir_path, char *backup_dir_path);//directory backup with files
int hash_check(char *file_path, char *backup_dir);//hash code check

int main(int argc, char *argv[])
{
	// argv[0] = hash, [1] = home_dir, [2] = backup_dir, [3] = current_dir, [4] = filename, [5] = option
	...
	if(argv[4] == NULL){
		fprintf(stderr, "Usage1 : add <FILENAME> [OPTION]\n  -d : add directory recursive\n");
		exit(1);
	}

	if(argv[5] != NULL && strstr(argv[5],"-d") == 0){
		fprintf(stderr, "Usage1 : add <FILENAME> [OPTION]\n  -d : add directory recursive\n");
                exit(1);
	}
	//<filename> 구분
	...
	//backup dir without filename
	strcpy(dst_dir, backup_dir);
	...
	if(access(real_path, R_OK|W_OK) != 0){
                fprintf(stderr, "Usage2 : add <FILENAME> [OPTION]\n  -d : add directory recursive\n");
                exit(1);
        }
	//filename length
	...
	if(stat(real_path, &file_info) != 0){
		fprintf(stderr, "Usage3	: add <FILENAME> [OPTION]\n  -d : add directory recursive\n");
                exit(1);
	}
	...
	//디렉토리
	if(S_ISDIR(file_info.st_mode) == 1){
		backup_directory(real_path, dst_dir);
	}
	//파일
	else{
		char backup_filename[4096];
		get_backup_filename(backup_filename, real_path, dst_dir);
		mkdir(dst_dir, 0777);
		if(strncmp(argv[4], "/home", 5) == 0){ //절대경로
                if(strncmp(argv[4], home_dir, strlen(home_dir)) != 0 || strncmp(argv[4], backup_dir, strlen(backup_dir)) == 0){
                        fprintf(stderr, "\"%s\" can't be backuped\n", argv[4]);
		...
		backup_file(real_path, backup_filename, dst_dir);
	}
...
// 파일 복사
void backup_file(char *src_path, char *dst_path, char *backup_dir) {
    int src_fd, dst_fd, n;
    char buf[1024];
...
// 디렉토리 복사
void backup_directory(char *src_path, char *dst_path) {
    DIR *src_dir, *dst_dir;
    ...
    while ((dir_entry = readdir(src_dir)) != NULL) {
        ...
        if (S_ISDIR(dir_stat.st_mode)){
            	backup_directory(src_file_path, dst_file_path);
        } else {
            	get_backup_filename(backup_filename, src_file_path, dst_path);
            	backup_file(src_file_path, dst_file_path, dst_path);
        }
    }
