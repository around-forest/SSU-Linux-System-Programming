...
int main()
{
		...
		for(i = 0; i < THREAD_NUM; i++) {
				...
				if(pthread_create(&tid[i], NULL, printhello, (void*)&thread_data_array[i]) != 0) {
								fprintf(stderr, "pthread_create error\n");
								exit(1);
				} // create new thread operating printhello func
		}
		sleep(5);
		exit(0);
}
...
void *printhello(void *arg) {
		...
		sum = data -> sum;
		message = data -> message; // print argument's tid, message, sum
		printf("Trhead %d: %s Sum=%d\n", thread_index, message, sum);
		return NULL;
}
